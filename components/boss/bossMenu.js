...
module.exports = {
	...
	execute(interaction) {
        ...
        switch(interaction.values[0]){
            case 'megapithecus':
                interaction.update({embeds: [newEmbedMonkey]})
            break;

            case 'broodgenetrix':
                interaction.update({embeds: [newEmbedSpider]})
            break;

            case 'beelzemorbus':
                interaction.update({embeds: [newEmbedFrog]})
            break;

            case 'cubozoa':
                interaction.update({embeds: [newEmbedCubozoa]})
            break;

            case 'obsidioequus':
                interaction.update({embeds: [newEmbedChali]})
            break;

            case 'dodobitus':
                interaction.update({embeds: [newEmbedDodo]})
            break;

            case 'doedicurus':
                interaction.update({embeds: [newEmbedDoed]})
            break;

            case 'gulabeetle':
                interaction.update({embeds: [newEmbedGula]})
            break;

            case 'argentustus':
                interaction.update({embeds: [newEmbedArgy]})
            break;

            case 'noctis':
                interaction.update({embeds: [newEmbedNoctis]})
            break;
        }

    ...
    async execute(interaction) {
        ...
        const prefix2 = (() => {
          ...
          switch(interaction.options._hoistedOptions[0]?.value){
            case "ruins":
                msg = await interaction.reply({embeds: [newEmbedRuins], components: [ruinsRow, ruinsTraps], fetchReply: true}) 
            break;
            
            case "ice":
                msg = await interaction.reply({embeds: [newEmbedIce], components: [iceRow, iceTraps], fetchReply: true}) 
            break;

            case "swamp":
                msg = await interaction.reply({embeds: [newEmbedSwamp], components: [swampRow, swampTraps], fetchReply: true}) 
            break;

            default: msg = await interaction.reply({embeds: [newEmbed], components: [defaultRow], fetchReply: true})
        }

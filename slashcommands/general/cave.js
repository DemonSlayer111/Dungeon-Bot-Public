...
module.exports = {
	name: 'cave',
	description: 'Information about ARK Mobile Caves',
    async execute(interaction) {
        ...
        .setColor('#0e48b8')
        .setDescription(`__**Caves**__\n\n**Overview**\nCaves are challenges available to any player brave enough to enter. There are 6 caves in ARK Mobile, each with its own set of challenges and rewards. Each cave has its own entrance on The Island from which it can be accessed. Caves do not have a time limit, but prolonged journeys may reduces the chances of survival. Creatures may also enter caves if they can fit through the entrance.\n\n**Rewards**\nThe main reward of a cave is its Artifact. Each cave has an artifact located within it, requiring players to get past the threats of the cave to obtain it. Caves may also contain loot crates which will reward players that open them with items or blueprints.\n\n**Threats**\nThe main threats in caves are high level creatures found within. Most artifacts and loot crates are heavily guarded by these creatures and will generally need to be defeated before artifacts and other loot can be claimed. Some caves may also have environmental hazards that can damage or kill unsuspecting survivors.`)
        .setImage("https://cdn.discordapp.com/attachments/760287836074278963/895055366772101210/Cave_Coords.png")
        ...
        switch(interaction.options._hoistedOptions[0]?.value){
            case 'south':
                msg = await interaction.reply({embeds: [embedSouth], components: [caveRow, southRow], fetchReply: true})
            break;
            case "central":
                msg = await interaction.reply({embeds: [embedCentral], components: [caveRow, centralRow], fetchReply: true})
            break;
            case 'lava':
                msg = await interaction.reply({embeds: [embedLava], components: [caveRow, lavaRow], fetchReply: true})
            break;
            case 'northwest':
                msg = await interaction.reply({embeds: [embedNorthwest], components: [caveRow, northwestRow], fetchReply: true})
            break;
            case 'yeti':
                msg = await interaction.reply({embeds: [embedYeti], components: [caveRow, yetiRow], fetchReply: true})
            break;
            case 'swamp':
                msg = await interaction.reply({embeds: [embedSwamp], components: [caveRow, swampRow], fetchReply: true})
            break;
            default:
                msg = await interaction.reply({embeds: [embed], components: [caveRow], fetchReply: true})
        }

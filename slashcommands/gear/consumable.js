...
module.exports = {
    ...
    switch(interaction.options._hoistedOptions[0]?.value){
        case "focal":
            msg = await interaction.reply({embeds: [embedFocal], components: [actionRow], fetchReply: true }) 
        break;
        case "lazarus":
            msg = await interaction.reply({embeds: [embedLazarus], components: [actionRow], fetchReply: true }) 
        break;

        case "enduro":
            msg = await interaction.reply({embeds: [embedEnduro], components: [actionRow], fetchReply: true }) 
        break;

        case "calien":
            msg = await interaction.reply({embeds: [embedCalien], components: [actionRow], fetchReply: true }) 
        break;

        case "fria":
            msg = await interaction.reply({embeds: [embedFria], components: [actionRow], fetchReply: true }) 
        break;

        case "medical":
            msg = await interaction.reply({embeds: [embedMedical], components: [actionRow], fetchReply: true }) 
        break;

        case "energy":
            msg = await interaction.reply({embeds: [embedEnergy], components: [actionRow], fetchReply: true }) 
        break;

        case "tartare":
            msg = await interaction.reply({embeds: [embedTartare], components: [actionRow], fetchReply: true }) 
        break;

        case "mindwipe":
            msg = await interaction.reply({embeds: [embedMindWipe], components: [actionRow], fetchReply: true }) 
        break;

        case "hemostatic":
            msg = await interaction.reply({embeds: [embedHemostatic], components: [actionRow], fetchReply: true }) 
        break;
        ...
        case "antidote":
            msg = await interaction.reply({embeds: [embedAntidote], components: [actionRow], fetchReply: true }) 
        break;

        case "stimulant":
            msg = await interaction.reply({embeds: [embedStimulant], components: [actionRow], fetchReply: true }) 
        break;
